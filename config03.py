...
class MaterialParams:
    ...
    def E_star(self) -> float:
        ...
        return 1.0 / ((1 - self.nu_indentor**2) / self.E_indentor + (1 - self.nu_half_space**2) / self.E_half_space)


@dataclass
class ContactParams:
    """Geometry and loading of the line contact"""
    a: float = 0.02        # [m] contact radius
    roller_seperation: float = 0.08 # [m] distance bettwen roller midpoints#######################################################################
    p_max: float = 1000    # [N/m^2]
    number_of_rollers = 1 ##########################################################################
...
seperation_distance_config = Config()
seperation_distance_config.contact.roller_seperation = 0.16
seperation_distance_config.contact.number_of_rollers = 3
